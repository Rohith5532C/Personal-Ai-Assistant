...
class MainWindow(QMainWindow):
      ...
      def initUI(self):
            ...
            screen_width = desktop.screenGeometry().width()
            screen_height = desktop.screenGeometry().height()
            stracked_widget = QStackedWidget(self)
            initial_screen = InitialScreen()
            message_screen = MessageScreen()
            stracked_widget.addWidget(initial_screen)
            stracked_widget.addWidget(message_screen)
            self.setGeometry(0, 0, screen_width, screen_height)
            self.setStyleSheet("background-color: black;")
            top_bar = CustomTopBar(self, stracked_widget)
            self.setMenuWidget(top_bar)
            self.setCentralWidget(stracked_widget)
def InitializeMicFile():
  if not os.path.exists(TempDirectoryPath('Mic.data')):
      with open(TempDirectoryPath('Mic.data'), 'w', encoding='utf-8') as file:
        file.write("False")  # Default to inactive           
...
if __name__ == "__main__":
      InitializeMicFile()  # Initialize Mic.data file if not exists
